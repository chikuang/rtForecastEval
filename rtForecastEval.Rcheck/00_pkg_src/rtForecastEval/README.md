...
**Repository:**
[github.com/chikuang/rtForecastEval](https://github.com/chikuang/rtForecastEval)
...
| Plotting | `plot_pcb()` (naive pointwise **skill** band); see README below for a simple **calibration / reliability** ggplot |
...
CRAN). After installation, load **`library(rtForecastEval)`**.
...
Alternative: `remotes::install_github("chikuang/rtForecastEval")` after
...
[`plot_pcb()`](https://github.com/chikuang/rtForecastEval/blob/main/R/plot_pcb.R)
plots the **pointwise mean loss difference** (relative **skill**) with a
naive normal band — not a classical **calibration** diagram.

A complementary check is **marginal calibration** at one time: bin
predicted probabilities at a single grid (here, closest to mid-game, (t
)) and compare mean outcome to mean forecast (reliability diagram). Full
**calibration surfaces** from the paper are in the NBA replication code;
this is a minimal ggplot using the same long-format columns (`phat_A`,
`phat_B`, `Y`, `grid`).
...
ggplot(rel_binned, aes(mean_forecast, mean_outcome)) +
  ...
  labs(
    ...
    subtitle = paste0("Grid = ", signif(g_mid, 3), "; 10 quantile bins per forecaster"),
...
grids, centered eigenvalues, and the same figures with narrative), open
the vignette:
